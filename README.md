...
- The column-splitting and table-parsing logic uses fixed coordinate zones
  tuned against two real invoice samples (different template generations).
  If Amazon changes their invoice template layout significantly in the
  future, some fields might fail to extract — if that happens, send the
  PDF and it's usually a quick, targeted fix rather than a rewrite
...
- `InvoiceParser.kt` — uses word positions to: split the header into seller
  ...
  invoices within one PDF, and parse the line-items table (handles tax
