...
class InvoiceParser(private val context: Context) {
    ...
    /**
     * Returns true only if the text contains at least one STRONG structural
     * signal (a money value, percentage, tax type, ASIN, or HSN code).
     * These are unambiguous markers that real numeric/identifier data is
     * embedded in this token. A bare integer alone is NOT a strong signal,
     * since plain description text often contains numbers (e.g. "500Ml",
     * "12 In 1", "2.25 L") that must never be treated as structured data.
     */
    private fun looksLikeSquishedStructuredToken(text: String): Boolean {
        return Regex("₹[\\d,]+\\.\\d{2}").containsMatchIn(text) ||
            Regex("\\d+(?:\\.\\d+)?%").containsMatchIn(text) ||
            Regex("\\b(CGST|SGST|IGST|UTGST)\\b").containsMatchIn(text) ||
            Regex("\\b[A-Z0-9]{10}\\b").containsMatchIn(text) ||
            Regex("HSN:\\d+").containsMatchIn(text)
    }

    /**
     * Classifies a token by CONTENT PATTERN first (money, rate%, tax type,
     * ASIN, HSN). If the token doesn't match any single pattern exactly,
     * this ALWAYS attempts to split it as a squished multi-field token
     * (e.g. "B01IBTD4HC₹36.00" - an ASIN glued directly to a unit price
     * with no separator) BEFORE falling back to position-based
     * classification - gated by [looksLikeSquishedStructuredToken] so a
     * plain description fragment that happens to contain a number (like
     * "(500Ml") is never mistaken for structured data.
     *
     * This ordering matters: an earlier version checked x0-position
     * BEFORE attempting the squish-split, which meant a squished token
     * whose starting x-position happened to fall within the description
     * zone (exactly the case for an ASIN+price glued together with no
     * separator) would short-circuit straight to "description" and never
     * get split at all - silently corrupting unit price, net amount, and
     * total for that row. Confirmed via on-device debug captures showing
     * PdfBox-Android producing this exact squished token.
     *
     * Returns a list of (kind, value) pairs since one input token can
     * expand into several pieces.
     */
    private fun classifyToken(text: String, x0: Float, descriptionColMaxX: Float): List<Pair<String, String>> {
        if (Regex("^₹[\\d,]+\\.\\d{2}$").matches(text)) return listOf("amount" to text)
        Regex("^(\\d+(?:\\.\\d+)?)%$").find(text)?.let { return listOf("rate" to it.groupValues[1]) }
        if (text in taxTypes) return listOf("type" to text)
        if (Regex("^[A-Z0-9]{10}$").matches(text)) return listOf("asin" to text)
        Regex("^HSN:(\\d+)$").find(text)?.let { return listOf("hsn" to it.groupValues[1]) }
        Regex("^(\\d+)$").find(text)?.let {
            return if (x0 >= descriptionColMaxX) {
                listOf("int" to it.groupValues[1])
            } else {
                listOf("description" to text)
            }
        }

        if (looksLikeSquishedStructuredToken(text)) {
            val split = splitSquishedToken(text)
            if (split.isNotEmpty()) {
                return split.map { (value, kind) -> kind to value }
            }
        }
    ...
    private fun parseItemBlock(
        ...
        for (row in rows) {
            for (w in row) {
                ...
                val classified = classifyToken(text, w.x0, descriptionColMaxX)
                ...
                for ((kind, value) in classified) {
                    when (kind) {
                        ...
                        "hsn" -> hsn = value
                        "unknown" -> {
                            // truly unrecognized text in the data zone - ignore
                        }
