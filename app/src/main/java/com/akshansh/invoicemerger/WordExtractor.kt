...
class WordExtractor : PDFTextStripper() {
    ...
    /**
     * Debug helper: returns the raw extracted characters (before word
     * grouping) for every page, as plain text lines of
     * "text\tx0\tx1\ttop" - one per character. Used to diagnose
     * extraction discrepancies between this on-device extractor and the
     * Python/pdfplumber prototype used to validate the parsing logic.
     */
    fun dumpRawCharsByPage(document: PDDocument): List<String> {
        pageChars.clear()
        for (pageIndex in 0 until document.numberOfPages) {
            currentPageChars = mutableListOf()
            startPage = pageIndex + 1
        ...
        return pageChars.map { chars ->
            chars.joinToString("\n") { c ->
                "${c.text}\t${c.x0}\t${c.x1}\t${c.top}"
            }
        }
    }
