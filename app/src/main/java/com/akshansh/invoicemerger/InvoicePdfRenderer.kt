...
class InvoicePdfRenderer(private val context: Context) {
    ...
    private fun drawSellerBillingShipping(canvas: Canvas, invoice: MergedInvoice, startY: Float): Float {
        ...
        master.billingStateCode?.let {
            drawLabelValue(canvas, "State/UT Code: ", it, rightX, rightY)
            rightY += 14f
        }
        ...
        master.placeOfSupply?.let {
            drawLabelValue(canvas, "Place of supply: ", it, leftX, y)
            y += 11f
        }
        master.placeOfDelivery?.let {
            drawLabelValue(canvas, "Place of delivery: ", it, leftX, y)
            y += 14f
        }
    ...
    private fun drawOrderInvoiceMeta(canvas: Canvas, invoice: MergedInvoice, startY: Float): Float {
        ...
        drawLabelValue(canvas, "Invoice Number : ", master.invoiceNumber, rightX, y)
        y += 11f
        drawLabelValue(canvas, "Order Date: ", master.orderDate, leftX, y)
        drawLabelValue(canvas, "Invoice Details : ", master.invoiceDetails, rightX, y)
        y += 11f
        drawLabelValue(canvas, "Invoice Date : ", master.invoiceDate, rightX, y)
        y += 18f
